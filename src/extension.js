...
async function onMessage(m) {
  ...
  switch (m.type) {
    ...
    case 'interrupt':
      ...
      break;
    // 'discard' (P06-S06) is GONE — D-059. Its modal promised "edits made since the step started",
    // but the checkpoint rewind was only reachable on Claude with a live session; every other case
    // (Codex, an errored turn that already tore the session down, canRewind:false) fell through to
    // `git checkout -- .` on the whole repo, which also threw away uncommitted work the owner did
    // before the run. Source Control does this properly, per file, with a diff you read first.
